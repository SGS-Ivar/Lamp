...
namespace Lamp
{
	...
	{
	public:
		static std::pair<bool, float> SAT(Ref<ConvexPolygonColliderComponent>& pCollA, Ref<ConvexPolygonColliderComponent>& pCollB)
		{
			auto polyA = pCollA;
			auto polyB = pCollB;
			...
			//glm::vec2 vec = pCollB->GetOwner()->GetPosition() - pCollA->GetOwner()->GetPosition();
			//float s = sqrtf(vec.x * vec.x + vec.y * vec.y);

			//if (pCollA->GetMass() > pCollB->GetMass())
			//{
			//	pCollB->GetOwner()->SetPosition(pCollB->GetOwner()->GetPosition() - glm::vec3((overlap * vec.x / s), (overlap * vec.y / s), 0));
			//}
			//else
			//{
			//	pCollA->GetOwner()->SetPosition(pCollA->GetOwner()->GetPosition() - glm::vec3((overlap * vec.x / s), (overlap * vec.y / s), 0));
			//}
				...
				for (size_t n = m + 1; n < LevelHandler::GetCurrent()->GetEntityManager()->GetEntities().size(); n++)
				{
					if (auto pCollA = LevelHandler::GetCurrent()->GetEntityManager()->GetEntities()[m]->GetComponent<ConvexPolygonColliderComponent>())
					{
						if (auto pCollB = LevelHandler::GetCurrent()->GetEntityManager()->GetEntities()[n]->GetComponent<ConvexPolygonColliderComponent>())
						{
							...
							{
								if (auto it = std::find(pCollA->GetCurrentColliders().begin(), pCollA->GetCurrentColliders().end(), pCollB); it == pCollA->GetCurrentColliders().end())
								{
									pCollA->GetCurrentColliders().push_back(pCollB);
									{
										if (pCollB->GetOwner() != NULL)
										{
											EntityCollisionEvent event(pair.second, pCollB->GetOwner(), pCollB->GetTag());
											pCollA->GetOwner()->OnEvent(event);
										}
										else 
										{
											EntityCollisionEvent event(pair.second, nullptr, "null");
											pCollA->GetOwner()->OnEvent(event);
										}
									}
								}

								if (auto it = std::find(pCollB->GetCurrentColliders().begin(), pCollB->GetCurrentColliders().end(), pCollA); it == pCollB->GetCurrentColliders().end())
								{
									if (pCollA->GetOwner() != NULL)
									{
										EntityCollisionEvent event(pair.second, pCollA->GetOwner(), pCollA->GetTag());
							...
							else if (!pCollA->GetIsColliding() && !pCollB->GetIsColliding())
							{
								if (auto it = std::find(pCollA->GetCurrentColliders().begin(), pCollA->GetCurrentColliders().end(), pCollB); it != pCollA->GetCurrentColliders().end())
								{
									...
									if (pCollB->GetOwner() != NULL)
									{
										EntityStoppedCollisionEvent event(pair.second, pCollB->GetOwner(), pCollB->GetTag());
										pCollA->GetOwner()->OnEvent(event);
									}
									else 
									{
										EntityStoppedCollisionEvent event(pair.second, nullptr, "");
										pCollA->GetOwner()->OnEvent(event);
									}
								}
								...
								if (auto it = std::find(pCollB->GetCurrentColliders().begin(), pCollB->GetCurrentColliders().end(), pCollA); it != pCollB->GetCurrentColliders().end())
								{
									...
									if (pCollA->GetOwner() != NULL)
									{
										EntityStoppedCollisionEvent event(pair.second, pCollA->GetOwner(), pCollA->GetTag());
										pCollB->GetOwner()->OnEvent(event);
									}
									else
									{
