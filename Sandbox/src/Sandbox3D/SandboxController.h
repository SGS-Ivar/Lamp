...
namespace Sandbox3D
{
	...
	public:
		SandboxController()
			: m_Listener(0)
		{
			m_CameraController = CreateRef<Lamp::PerspectiveCameraController>(Lamp::LevelHandler::GetCurrent()->GetEnvironment().CameraFOV, 0.1f, 100.f);
			m_CameraController->SetPosition(Lamp::LevelHandler::GetCurrent()->GetEnvironment().CameraPosition);
			m_CameraController->SetRotation(Lamp::LevelHandler::GetCurrent()->GetEnvironment().CameraRotation);
		}
		...
		void Update(Lamp::Timestep ts)
		{
			...
			Lamp::LevelHandler::GetCurrent()->GetEnvironment().CameraPosition = m_CameraController->GetCamera()->GetPosition();
			Lamp::LevelHandler::GetCurrent()->GetEnvironment().CameraRotation = m_CameraController->GetCamera()->GetRotation();
			Lamp::LevelHandler::GetCurrent()->GetEnvironment().CameraFOV = m_CameraController->GetFOV();
